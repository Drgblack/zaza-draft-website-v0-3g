...
// Company page translations - English (with escaped quotes)
const companyEN = `
  // Company page
...
// Company page translations - German (with escaped quotes)
const companyDE = `
  // Company page
...
// Find insertion points (try multiple variations)
let enInsertPoint = content.indexOf('"contact.help.support": "→ Support Centre",');
if (enInsertPoint === -1) {
  enInsertPoint = content.indexOf('"contact.help.cta": "Get Started →",');
}

if (enInsertPoint > -1) {
  const afterComma = content.indexOf(',', enInsertPoint) + 1;
  content = content.slice(0, afterComma) + companyEN + content.slice(afterComma);
...
// German insertion
let deInsertPoint = content.lastIndexOf('"contact.help.support": "→ Support-Center",');
if (deInsertPoint === -1) {
  deInsertPoint = content.lastIndexOf('"contact.help.cta": "Jetzt starten →",');
}

if (deInsertPoint > -1) {
  const afterComma = content.indexOf(',', deInsertPoint) + 1;
  content = content.slice(0, afterComma) + companyDE + content.slice(afterComma);
...
console.log('✅ Company page translations added!');