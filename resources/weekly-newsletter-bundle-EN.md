# "This Week in Our Class" Newsletter Starter Kit
## Weekly Parent Update Templates That Actually Get Read

*Stop stressing about weekly newsletters. This kit includes 3 ready-to-use templates (elementary, middle, and high school), a content bank with 50+ pre-written sections, and a 10-minute production system.*

**Author:** Dr. Greg Blackburn  
**Website:** www.zazadraft.com

---

## Why Weekly Newsletters Work

You know parent communication matters. But long emails get ignored, and individual updates take forever. A weekly newsletter hits the sweet spot:

- **One message reaches everyone** (instead of 25 separate emails)
- **Parents stay informed** without overwhelming you
- **Builds community** by showcasing student work and celebrations
- **Prevents "I didn't know" excuses** at conference time

This kit gives you:
- **3 template formats** designed for different grade levels
- **A content bank** with 50+ pre-written sections you can copy-paste
- **A 10-minute system** for producing newsletters without burning out
- **Subject line formulas** that increase open rates

> **Who this is for:** K-12 teachers who want to improve parent communication, build classroom community, and reduce "Did you know about...?" emails - without adding hours to their workload.

---

## The 3 Newsletter Templates

### Template 1: Elementary Edition (Grades K-5)

**Best for:** Visual learners (parents and kids), classes with lots of hands-on activities, younger students who can't always communicate what happened at school.

**Format:**
```
Subject: This Week in [Grade/Class] - [Date Range]

ðŸ“š THIS WEEK WE LEARNED:
[2-3 bullet points about content covered]

ðŸŒŸ CLASSROOM HIGHLIGHTS:
[1-2 specific student achievements or fun moments]

ðŸ“ UPCOMING:
[Dates for tests, projects, field trips, or events]

ðŸ’¡ HOW YOU CAN HELP AT HOME:
[1 specific, doable suggestion]

ðŸ“¸ SNAPSHOTS:
[Optional: 1-2 photos of classroom activities]

REMINDERS:
[Quick hits: library day, PE shoes, picture day, etc.]
```

**Sample:**
```
Subject: This Week in 3rd Grade - November 6-10

ðŸ“š THIS WEEK WE LEARNED:
- Math: Multiplication facts (6, 7, 8 times tables)
- Reading: Character traits and using text evidence
- Science: States of matter - solids, liquids, gases

ðŸŒŸ CLASSROOM HIGHLIGHTS:
- Maya explained her science experiment so clearly that even I learned something new!
- Our class read a combined 847 minutes this week - a new record!

ðŸ“ UPCOMING:
- Nov 15: Multiplication quiz (facts 0-10)
- Nov 17: Book reports due
- Nov 22-24: Thanksgiving break (no school)

ðŸ’¡ HOW YOU CAN HELP AT HOME:
Practice multiplication facts for 5 minutes during dinner or car rides. Try the "Around the World" game: you say "6 x 7," and your child races to answer before the timer!

REMINDERS:
- Library books due Monday
- Spirit Week starts Nov 13 (details coming soon)
- Parent-teacher conferences: sign up by Friday
```

---

### Template 2: Middle School Edition (Grades 6-8)

**Best for:** Content-heavy classes, multiple sections/periods, students who need structure and accountability.

**Format:**
```
Subject: [Subject] Update - Week of [Date]

WHAT WE COVERED THIS WEEK:
[Brief learning objectives or topics]

ASSESSMENT ALERT:
[Upcoming quizzes, tests, or projects]

SKILLS IN FOCUS:
[1-2 key skills or habits we're working on]

STUDENT SHOUT-OUTS:
[Optional: recognize 2-3 students for growth or effort]

WHAT TO REVIEW AT HOME:
[Specific content to reinforce]

UPCOMING DATES:
[Calendar items]

HAVE QUESTIONS? 
[Office hours, email, or contact info]
```

**Sample:**
```
Subject: 7th Grade Math - Week of November 6

WHAT WE COVERED THIS WEEK:
- Solving two-step equations (e.g., 3x + 5 = 20)
- Checking solutions by substituting back into equations
- Real-world applications: budgeting and distance problems

ASSESSMENT ALERT:
- Friday, Nov 10: Quiz on two-step equations (15 points)
- Nov 17: Unit 3 test on all equation types

SKILLS IN FOCUS:
This week we practiced "showing our work" step-by-step. Many students are getting correct answers but skipping steps, which makes it hard to catch mistakes. We're building that habit together!

STUDENT SHOUT-OUTS:
- Marcus: Helped three classmates understand the "distributive property" during group work
- Zoe: Showed huge growth in checking her work before submitting
- Table 4: Best teamwork during the budgeting challenge!

WHAT TO REVIEW AT HOME:
Practice problems are on Google Classroom (Unit 3 Practice Set). Aim for 10-15 minutes, 3-4 times this week. Focus on problems 8-15.

UPCOMING DATES:
- Nov 10: Quiz
- Nov 13: No school (professional development day)
- Nov 17: Unit test

HAVE QUESTIONS?
Email me or drop by room 204 during office hours (Tuesday/Thursday, 3-3:30 PM).
```

---

### Template 3: High School Edition (Grades 9-12)

**Best for:** Older students, AP/honors classes, content that requires independence and self-advocacy.

**Format:**
```
Subject: [Course Name] - Weekly Update [Date]

THIS WEEK IN CLASS:
[Key concepts or content covered]

LOOKING AHEAD:
[Major assessments, deadlines, or calendar items]

STUDY TIP OF THE WEEK:
[One actionable strategy for success]

RESOURCES & SUPPORT:
[Where to find help: office hours, tutoring, online materials]

ACCOUNTABILITY CHECK:
[Optional: reminders about missing work or grade updates]
```

**Sample:**
```
Subject: AP Biology - Weekly Update (Nov 6-10)

THIS WEEK IN CLASS:
We wrapped up cellular respiration and started photosynthesis. Key focus: understanding the relationship between the two processes and how energy flows through living systems. We also analyzed data from the yeast fermentation lab.

LOOKING AHEAD:
- Nov 13: Lab report due (cellular respiration - upload to Google Classroom by 11:59 PM)
- Nov 17: Unit 4 test (cellular respiration + photosynthesis)
- Nov 20: Start Unit 5 (cell division and mitosis)

STUDY TIP OF THE WEEK:
Create a Venn diagram comparing photosynthesis and cellular respiration. Focus on: reactants, products, location in the cell, and energy flow. This visual will help you see the big picture.

RESOURCES & SUPPORT:
- Review videos posted on Google Classroom (watch before Friday's review session)
- Office hours: Mon/Wed 2:45-3:30 PM in room 312
- Study group forming for the unit test - see posted sign-up sheet

ACCOUNTABILITY CHECK:
A few reminders:
- 8 students still need to submit last week's reading guide (10% penalty if submitted by Nov 8)
- Check your grades on Infinite Campus - reach out if you have questions about any scores
```

---

## The Content Bank: Copy-Paste Sections

Use these pre-written sections when you're short on time:

### Learning Highlights (Pick 1-2):
- "This week, we explored [topic] by [activity]. Students practiced [skill] and learned [key takeaway]."
- "We dove deep into [concept]. Ask your student to explain [specific example] - they might surprise you with how much they know!"
- "Students worked collaboratively on [project]. The focus was on [skill like teamwork, critical thinking, creativity]."

### Student Celebrations (Pick 1-2):
- "[Student name] demonstrated excellent [skill/behavior] when they [specific example]."
- "Shout-out to [student name] for [improvement/achievement]. Keep it up!"
- "This week's class MVP: [Student name] for [reason]."
- "Our class showed amazing growth in [area]. I'm proud of the effort everyone is putting in."

### How You Can Help at Home:
- "Practice [skill] for 10 minutes, 3 times this week. Here's how: [specific method]."
- "Ask your child to teach you about [topic]. Teaching is one of the best ways to solidify learning!"
- "Review [specific content] together. Your child has access to [resource] on [platform]."
- "Encourage your child to complete [assignment] in small chunks (15 minutes per day) instead of all at once."

### Assessment Alerts:
- "Heads up: [Assessment type] on [date] covering [topics]. Study guide available on [platform]."
- "[Project name] is due [date]. Rubric and instructions are posted on [platform]. Remind your child to start early!"
- "Unit test next [day]. Students should review [specific materials]."

### Reminders:
- "Don't forget: [Event] on [date]."
- "Mark your calendars: [Upcoming event with details]."
- "Quick reminder: [Library day/PE clothes/Field trip permission slips/etc.]"

### Encouraging Closes:
- "Thank you for your partnership in your child's learning. Reach out anytime!"
- "I'm excited about what we're learning together. Let me know if you have questions!"
- "Your support makes all the difference. Have a great week!"
- "Looking forward to another great week of learning!"

---

## The 10-Minute Newsletter System

### Sunday Night Routine (10 minutes total):

**Minutes 1-3: Gather Content**
- Open your lesson plans or planner
- List 3 topics you covered this week
- Note 1-2 upcoming dates or deadlines

**Minutes 4-6: Choose Template & Fill**
- Pick your grade-level template
- Copy-paste from the content bank (don't write from scratch!)
- Plug in specific details (student names, dates, topics)

**Minutes 7-9: Review & Customize**
- Read through once (catch typos, check tone)
- Add 1 personal touch (a specific student moment or your own tip)

**Minute 10: Send & Celebrate**
- Send to your email list or post to your LMS
- Done!

> ðŸ’¡ **Pro Tip:** Batch-write newsletters. On Sunday, draft newsletters for the next 2-3 weeks by leaving blanks for specific details. On Monday, fill in the blanks and send.

---

## Subject Lines That Get Opened

### For Elementary:
- "This Week in [Grade] - [Date Range]"
- "[Teacher Name]'s Class Update - [Week of Date]"
- "What We Learned This Week + Upcoming Fun!"

### For Middle School:
- "[Subject] Update - Week of [Date]"
- "This Week in [Subject]: [Catchy Topic]"
- "[Course Name] - Weekly Snapshot"

### For High School:
- "[Course Name] Weekly Update [Date]"
- "What's Happening in [Subject] - [Week of Date]"
- "[Course] This Week: [Big Topic or Assessment]"

**Avoid:**
- Generic subject lines ("Update" or "Newsletter")
- All caps ("IMPORTANT UPDATE!")
- Overly long subject lines (keep under 50 characters)

---

## When to Send Your Newsletter

### Best Days:
- **Sunday evening (6-8 PM):** Parents are planning the week ahead
- **Monday morning (6-7 AM):** Lands in inbox before the work rush
- **Friday afternoon (3-5 PM):** Wraps up the week, gives weekend review time

### Worst Days:
- **Wednesday/Thursday:** Mid-week chaos, easy to miss
- **Late Friday evening:** Gets buried in weekend emails
- **Monday evening:** Competes with after-school rush

> ðŸ’¡ **Pro Tip:** Be consistent. Pick one day and time, and stick to it. Parents will come to expect and look for your newsletter.

---

## Common Newsletter Mistakes (And How to Avoid Them)

### Mistake #1: Too Long
**Problem:** Parents don't read past paragraph 2.

**Fix:** Keep it under 300 words. If you can't, break it into sections with headers.

---

### Mistake #2: Too Vague
**Problem:** "We learned about science this week" doesn't tell parents anything useful.

**Fix:** Be specific. "We explored states of matter by freezing water, melting ice, and observing evaporation."

---

### Mistake #3: All Reminders, No Learning
**Problem:** Newsletter reads like an admin memo (no celebration of learning).

**Fix:** Follow the 70-30 rule: 70% learning highlights, 30% logistics.

---

### Mistake #4: No Personality
**Problem:** Newsletter sounds like a robot wrote it.

**Fix:** Add one personal touch each week: a funny student quote, a teaching moment, or a quick reflection.

---

## Customization Ideas

### Add Photos (Elementary):
- Include 1-2 classroom activity photos
- Use a tool like Canva to create a simple photo collage
- Always get photo permission first (check your school's policy)

### Add Student Work Samples (Middle School):
- Feature 1-2 exemplary assignments (with student permission)
- Include a brief "Why this is excellent" caption
- Rotate which students are featured

### Add Study Resources (High School):
- Link to review videos, study guides, or practice quizzes
- Embed a Google Doc with "Top 10 Study Tips for [Upcoming Test]"
- Share a student-created study tool (flashcards, Quizlet set, etc.)

---

## Sample 4-Week Newsletter Plan

**Week 1: Standard Update**  
Use your grade-level template. Focus on learning highlights and upcoming dates.

**Week 2: Student Spotlight Edition**  
Feature 3-5 student achievements. Celebrate growth, not just grades.

**Week 3: Study Tips Special**  
Share specific strategies for an upcoming assessment. Include practice resources.

**Week 4: Fun & Community**  
Highlight classroom culture, inside jokes, or a team-building moment. Remind parents why their kids love your class.

---

## Bonus: Newsletter Alternatives for Busy Weeks

When you don't have 10 minutes:

### Option 1: The 3-Bullet Email
```
Subject: Quick Update - [Date]

Hi families,

Three quick things:
1. We learned [topic] this week
2. [Assessment] coming up on [date]
3. Reminder: [logistical item]

Have a great week!
```

### Option 2: The Google Classroom Announcement
Post a brief update directly to your LMS (parents get notified automatically).

### Option 3: The Seesaw/ClassDojo Message
Use your class app to share a quick photo + caption.

> ðŸ’¡ **Pro Tip:** It's better to send something short than to skip a week. Consistency builds trust.

---

## FAQ

**Q: What if parents don't read my newsletters?**  
Try these strategies:
- Use a catchy subject line
- Keep it under 250 words
- Send at a consistent time
- Ask a question ("What did your child tell you about [topic]?") to prompt engagement

**Q: Should I send newsletters during breaks?**  
No. Parents need a break too. Resume the week you return to school.

**Q: What if I teach multiple sections?**  
Send one newsletter per course (not per period). Adjust content for each unique course you teach.

**Q: Can I use a tool to automate this?**  
Yes! Tools like Mailchimp, Google Forms + Sheets, or Zaza Draft can help you create templates and speed up production.

---

## Your Next Steps

### This Week:
1. Choose your template (elementary, middle, or high school)
2. Set a recurring Sunday night reminder (or pick your send day)
3. Draft your first newsletter using the content bank
4. Send it and track your open rate

### This Month:
- Stick to your send schedule for 4 consecutive weeks
- Gather parent feedback: "Is this newsletter helpful? What would make it better?"
- Refine your template based on what works

---

## Closing Thoughts

Parent communication doesn't have to be a time suck. With the right template, a content bank, and a 10-minute system, you can keep families informed, build community, and reduce the "I didn't know" emails - all without sacrificing your evenings.

**Ten minutes on Sunday saves ten interruptions during the week.**

---

**Author:** Dr. Greg Blackburn  
**Website:** www.zazadraft.com

*Â© 2025 Zaza Technologies. This resource is free for teachers. Share with your team, but please keep the attribution intact.*

