...
export function BlogPostClient({ post, relatedPosts }: BlogPostClientProps) {
  ...
  const content =
    language === "de" && post.contentDe ? post.contentDe : post.content;
  ...
  const parseContent = () => {
    ...
    try {
      ...
      while (remaining.length > 0 && safetyCounter < MAX_ITERATIONS) {
        ...
        const afterOpen = remaining.substring(
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        if (segment.type === "content") {
          const htmlContent = isHtmlContent
            ? addBlogClasses(segment.content)
            : convertMarkdownToHtml(segment.content);
          if (htmlContent.trim()) {
          ...
          elements.push(
            renderCallout(calloutType, segment.content, currentIndex++),
          );
          lastWasH2 = false;
  ...
  const convertMarkdownToHtml = (markdown: string): string => {
    ...
    html = html.replace(
      /^(\d+)\. (.+)$/gm,
      '<li class="blog-li-numbered">$2</li>',
    );
  ...
  const renderCallout = (type: string, content: string, index: number) => {
    const calloutConfig: Record<
      string,
      { icon: JSX.Element; title: string; className: string }
    > = {
      QUICK_WIN: {
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-12 lg:pt-6 lg:pb-16">
        ...
        <header className="mb-12">
          <div className="flex flex-wrap gap-2 mb-6">
            <Badge className="bg-[#2D5B8F] text-white border-0">
              {post.category}
            </Badge>
            {post.tags.slice(0, 3).map((tag) => (
              <Badge
                key={tag}
                variant="outline"
                className="border-[#6BBF9A] text-[#6BBF9A]"
              >
                {tag}
              </Badge>
            ))}
          ...
          <h1 className="text-4xl sm:text-5xl font-bold text-[#2C3E50] mb-6 leading-tight">
            {post.title}
          </h1>

          <ShareBar title={post.title} lang={language} />

          <p className="text-xl text-[#5A6C7D] mb-8 leading-relaxed">
            {post.excerpt}
          </p>
              ...
              {relatedPosts.map((relatedPost) => (
                ...
                >
                  <Card className="bg-white border-[#E5E7EB] hover:border-[#2D5B8F] hover:shadow-lg transition-all h-full p-6">
                    <Badge className="bg-[#F28B5F]/10 text-[#F28B5F] border-0 mb-3">
                      {relatedPost.category}
                    </Badge>
                    <h3 className="font-bold text-[#2C3E50] mb-2 line-clamp-2 group-hover:text-[#2D5B8F] transition-colors">
                      {relatedPost.title}
                    </h3>
                    <p className="text-sm text-[#5A6C7D] line-clamp-3">
                      {relatedPost.excerpt}
                    </p>
                  </Card>
                </Link>
