            ...
            return (
              <Card key={resource.slug} className="bg-[#0B1220] border-[#1F2937]">
                <CardContent className="p-6">
                  ...
                  {((((resource.blurb ?? DEFAULTS[resource.slug]?.blurb) ?? DEFAULTS[resource.slug]?.blurb) ?? DEFAULTS[resource.slug]?.blurb) ?? DEFAULTS[resource.slug]?.blurb) ? (
                    <p className="text-[#9CA3AF] mb-4">{((((resource.blurb ?? DEFAULTS[resource.slug]?.blurb) ?? DEFAULTS[resource.slug]?.blurb) ?? DEFAULTS[resource.slug]?.blurb) ?? DEFAULTS[resource.slug]?.blurb)}</p>
                  ) : null}
