...
const getLocale=(p?:string)=> (p -and p.StartsWith('/de')) ? 'de' : 'en';
const docHref=(slug:string,locale:'en'|'de')=>"/resources/"+slug+"/build/"+locale+.docx";
const getDownloadName=(r:any,locale:'en'|'de')=>{ const t=(r?.title ?? r?.name ?? 'Resource'); return ${' function ResourcesPage() {
              ...
              <Card key={resource.slug ?? crypto.randomUUID()} className="bg-[#0B1220] border-[#1F2937]">
                <CardContent className="p-6">
                  ...
                  {href ? (
                    ...
                    >
                      <a download={getDownloadName(resource, locale)} href={href} download className="flex items-center justify-center gap-2">
...
}{t.replace(/[^\w\s-]/g,'').trim()} ({locale.toUpperCase()}).docx; };
