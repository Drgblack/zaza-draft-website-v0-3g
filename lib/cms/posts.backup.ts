...
function mergePosts(curated: BlogPost[], imported: BlogPost[]): BlogPost[] {
  ...
  const combined = [...curated, ...imported].reduce<BlogPost[]>((acc, post) => {
    ...
    const lang = anyPost.language ?? "en";
    ...
    acc.push({
      ...post,
      publishedAt: normalizeDate(anyPost.publishedAt),
      language: lang,
    } as BlogPost);
...
const merged = mergePosts(curatedPosts as BlogPost[], readContentPosts());
export const posts: BlogPost[] = merged.length ? merged : (fallbackPosts as BlogPost[]);
...
export function getAllPostsByLanguage(language: "en" | "de"): BlogPost[] {
  return posts.filter((p: any) => (p.language ?? "en") === language);
}
