...
export function toBlogPosts(drafts: any[], meta: Array<Partial<BlogPost>>): BlogPost[] {
  return drafts.map((d, i) => {
    const block: InboxBlock = d?.blogPostContent ? d.blogPostContent : d?.en ? d : null

    if (!block) {
      throw new Error("Inbox draft has wrong shape; expected { blogPostContent: { en, de? } } or { en, de? }")
    }
    ...
    return {
      ...
      publishedAt: String(m.publishedAt ?? new Date().toISOString().slice(0, 10)),
